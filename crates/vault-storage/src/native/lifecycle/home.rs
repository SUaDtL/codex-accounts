...
impl Registration {
    fn reserve(key: [u8; 32]) -> Result<Self, StorageError> {
        if !HELD.lock().map_err(|_| StorageError::RecoveryRequired)?.insert(key) {
...
fn object_id(f: &File) -> Result<ObjectId, StorageError> {
    ...
    if unsafe {
        GetFileInformationByHandleEx(
            f.as_raw_handle(), FileIdInfo, (&mut info as *mut FILE_ID_INFO).cast(),
    ...
    Ok(ObjectId { volume: info.VolumeSerialNumber, file: info.FileId.Identifier })
}
fn owner_is_current(f: &File, security: &Security) -> Result<(), StorageError> {
    ...
    let result = unsafe {
        GetSecurityInfo(f.as_raw_handle(), SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION,
            &mut owner, null_mut(), null_mut(), null_mut(), &mut sd)
    };
    let _sd = Allocation(sd);
    if result != 0 { return Err(error(result)); }
    if sid_copy(owner)? != security.user { return Err(StorageError::UnsafePath); }
    Ok(())
}
fn kernel_descriptor(security: &Security) -> Result<Allocation, StorageError> {
    ...
    if unsafe { ConvertSidToStringSidW(security.user.as_ptr().cast_mut().cast(), &mut sid_text) } == 0 {
        return Err(last());
    }
    let sid_text = Allocation(sid_text.cast());
    let mut n = 0;
    while n < 256 && unsafe { *sid_text.0.cast::<u16>().add(n) } != 0 { n += 1; }
    if n == 256 { return Err(StorageError::UnsafePath); }
    let sid = String::from_utf16(unsafe { std::slice::from_raw_parts(sid_text.0.cast::<u16>(), n) })
        .map_err(|_| StorageError::UnsafePath)?;
    let sddl: Vec<u16> = format!("O:{sid}D:P(A;;0x{MUTEX_RIGHTS:08x};;;{sid})(A;;0x{MUTEX_RIGHTS:08x};;;SY)\0")
    ...
    if unsafe { ConvertStringSecurityDescriptorToSecurityDescriptorW(sddl.as_ptr(), SDDL_REVISION_1,
        &mut result, null_mut()) } == 0 { return Err(last()); }
    Ok(Allocation(result))
}
fn check_mutex(handle: HANDLE, expected: &Allocation, security: &Security) -> Result<(), StorageError> {
    let mut owner = null_mut();
    let mut dacl = null_mut();
    let mut sd = null_mut();
    // SAFETY: READ_CONTROL handle and OS-owned descriptor outputs.
    let status = unsafe { GetSecurityInfo(handle, SE_KERNEL_OBJECT,
        OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION, &mut owner, null_mut(),
        &mut dacl, null_mut(), &mut sd) };
    let actual = Allocation(sd);
    if status != 0 { return Err(error(status)); }
    if sid_copy(owner)? != security.user || dacl.is_null() { return Err(StorageError::UnsafePath); }
    let mut control = 0;
    let mut revision = 0;
    ...
    if unsafe { GetSecurityDescriptorControl(actual.0, &mut control, &mut revision) } == 0
        ...
        || unsafe { GetSecurityDescriptorDacl(expected.0, &mut present, &mut expected_dacl, &mut defaulted) } == 0
        || present == 0 || expected_dacl.is_null()
        || unsafe { IsValidAcl(dacl) } == 0 || unsafe { IsValidAcl(expected_dacl) } == 0 {
        return Err(StorageError::UnsafePath);
    }
    let size = unsafe { (*dacl).AclSize } as usize;
    let expected_size = unsafe { (*expected_dacl).AclSize } as usize;
    if !(8..=1024).contains(&size) || size != expected_size { return Err(StorageError::UnsafePath); }
...
impl std::fmt::Debug for HomeLock {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.write_str("HomeLock([REDACTED])") }
}
impl HomeLock {
    pub(super) fn acquire(path: &Path) -> Result<Self, StorageError> {
        ...
        paths.reverse();
        if paths.len() < 3 || paths.len() > 64 { return Err(StorageError::UnsafePath); }
        let mut directories = Vec::new();
        for p in paths {
            let file = open_file(p, FILE_READ_ATTRIBUTES | READ_CONTROL, FILE_SHARE_READ, OPEN_EXISTING, None)?;
        ...
        if unsafe { GetVolumeInformationByHandleW(leaf.as_raw_handle(), null_mut(), 0,
            null_mut(), null_mut(), null_mut(), fs.as_mut_ptr(), fs.len() as u32) } == 0 { return Err(last()); }
        if fs[..5] != [78, 84, 70, 83, 0] { return Err(StorageError::UnsafePath); }
        // Reuse the reviewed sync/path inspection without opening credential files.
        let handles: Vec<File> = directories.iter().map(|(_, f, _)| f.try_clone().map_err(|_| StorageError::Io))
            .collect::<Result<_, _>>()?;
        ...
        let name: Vec<u16> = format!("Global\\CodexAccounts.Home.v1.{hex}\0").encode_utf16().collect();
        let descriptor = kernel_descriptor(&security)?;
        let attrs = SECURITY_ATTRIBUTES { nLength: std::mem::size_of::<SECURITY_ATTRIBUTES>() as u32,
            lpSecurityDescriptor: descriptor.0, bInheritHandle: 0 };
        ...
        if handle.is_null() { return Err(last()); }
        ...
        let lock = Self { directories, security, mutex, registration, abandoned, _thread_bound: PhantomData };
        lock.revalidate()?;
        Ok(lock)
    }
    pub(super) fn abandoned(&self) -> bool { self.abandoned }
    pub(super) fn revalidate(&self) -> Result<(), StorageError> {
        for (path, file, expected) in &self.directories {
            validate_path(path)?;
            check_object(file, true)?;
            self.security.check(file, false)?;
            let reopened = open_file(path, FILE_READ_ATTRIBUTES | READ_CONTROL, FILE_SHARE_READ, OPEN_EXISTING, None)?;
        ...
        let handles: Vec<File> = self.directories.iter().map(|(_, f, _)| f.try_clone().map_err(|_| StorageError::Io))
            .collect::<Result<_, _>>()?;
        cloud_check(leaf, &handles)
    }
}
impl Drop for HomeLock {
    fn drop(&mut self) {
        ...
        if unsafe { ReleaseMutex(self.mutex.0) } == 0 { self.registration.remove = false; }
    }
}
