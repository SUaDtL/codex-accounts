...
impl RootKey {
    ...
    pub(crate) fn derive(&self, info: &[u8]) -> Result<Zeroizing<[u8; 32]>, CryptoError> {
        ...
        Hkdf::<Sha256>::new(Some(&self.id), self.secret.as_ref())
            .expand(info, key.as_mut()).map_err(|_| CryptoError::InputLimit)?;
        Ok(key)
    }
    fn fingerprint_mac(&self, context: &EnvelopeContext, bytes: Option<&[u8]>) -> Result<Hmac<Sha256>, CryptoError> {
    ...
    pub fn fingerprint(&self, context: &EnvelopeContext, bytes: Option<&[u8]>) -> Result<Fingerprint, CryptoError> {
        let mut tag = self.fingerprint_mac(context, bytes)?.finalize().into_bytes();
        let mut value = Zeroizing::new([0; 32]);
        value.copy_from_slice(&tag);
        tag.as_mut_slice().zeroize();
        Ok(Fingerprint(value))
    }
    pub fn verify_fingerprint(&self, expected: &Fingerprint, context: &EnvelopeContext, bytes: Option<&[u8]>) -> Result<(), CryptoError> {
        self.fingerprint_mac(context, bytes)?.verify_slice(expected.0.as_ref())
    ...
    pub fn unprotect(protected: &ProtectedRootKey) -> Result<Self, CryptoError> {
        ...
        #[cfg(not(all(windows, target_arch = "x86_64")))]
        { let _ = protected; Err(CryptoError::UnsupportedPlatform) }
    }
}
...
impl ProtectedRootKey {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, CryptoError> {
        if bytes.len() > MAX_PROTECTED_BYTES { return Err(CryptoError::InputLimit); }
        if bytes.len() < 11 || &bytes[..4] != PROTECTED_MAGIC { return Err(CryptoError::InvalidEnvelope); }
        if bytes[4] != 1 { return Err(CryptoError::UnsupportedVersion); }
        if bytes[5] != 1 { return Err(CryptoError::UnsupportedAlgorithm); }
        let len = u32::from_le_bytes(bytes[6..10].try_into().map_err(|_| CryptoError::InvalidEnvelope)?) as usize;
        if len != bytes.len() - 10 { return Err(CryptoError::InvalidEnvelope); }
    ...
    pub(crate) fn wrap(blob: &[u8]) -> Result<Self, CryptoError> {
        if blob.is_empty() || blob.len() > MAX_PROTECTED_BYTES - 10 { return Err(CryptoError::InputLimit); }
    ...
    #[cfg(all(windows, target_arch = "x86_64"))]
    pub(crate) fn blob(&self) -> &[u8] { &self.0[10..] }
}
...
mod tests {
    ...
    fn root_failure_returns_no_key() {
        assert!(matches!(RootKey::generate_using(&mut FailingEntropy), Err(CryptoError::RandomnessUnavailable)));
    }
    #[test]
    fn key_id_failure_also_returns_no_key() {
        ...
        impl Entropy for SecondFails {
            fn fill(&mut self, out: &mut [u8]) -> Result<(), CryptoError> {
                self.0 += 1;
                out.fill(0x53);
                if self.0 == 2 { Err(CryptoError::RandomnessUnavailable) } else { Ok(()) }
            }
        }
        assert!(matches!(RootKey::generate_using(&mut SecondFails(0)), Err(CryptoError::RandomnessUnavailable)));
    }
    #[test]
    fn encryption_and_fingerprint_keys_are_separate_and_key_id_bound() {
        ...
        let other = RootKey { secret: Zeroizing::new([0x53; 32]), id: [0x42; 16] };
