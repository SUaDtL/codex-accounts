...
fn transform(data: &[u8], encrypt: bool) -> Result<Zeroizing<Vec<u8>>, CryptoError> {
    if data.is_empty() || data.len() > 4096 { return Err(CryptoError::InputLimit); }
    ...
    let success = unsafe {
        if encrypt {
            CryptProtectData(&input, null(), &entropy, null(), null(), CRYPTPROTECT_UI_FORBIDDEN, &mut out)
        } else {
...
fn integrity(key: &RootKey, data: &[u8]) -> Result<Hmac<Sha256>, CryptoError> {
    ...
    let mut mac = <Hmac<Sha256> as Mac>::new_from_slice(derived.as_ref()).map_err(|_| CryptoError::KeyProtectionUnavailable)?;
    mac.update(data);
    Ok(mac)
}
...
pub(crate) fn unprotect(key: &ProtectedRootKey) -> Result<RootKey, CryptoError> {
    ...
    if inner.len() != INNER_BYTES || &inner[..8] != MAGIC { return Err(CryptoError::KeyProtectionUnavailable); }
    let mut secret = Zeroizing::new([0; 32]);
    secret.copy_from_slice(&inner[24..56]);
    let id: [u8; 16] = inner[8..24].try_into().map_err(|_| CryptoError::KeyProtectionUnavailable)?;
    if *secret == [0; 32] || id == [0; 16] { return Err(CryptoError::KeyProtectionUnavailable); }
    ...
    integrity(&root, &inner[..56])?.verify_slice(&inner[56..])
...
mod tests {
    ...
    fn native_dpapi_corruption_never_becomes_a_key() {
        let root = RootKey::generate().unwrap();
        let protected = protect(&root).unwrap();
        for index in [10, protected.as_bytes().len() / 2, protected.as_bytes().len() - 1] {
    ...
    fn native_dpapi_rejects_wrong_inner_shape_and_integrity() {
        for len in [1, INNER_BYTES - 1, INNER_BYTES, INNER_BYTES + 1] {
            ...
            if len >= 8 { data[..8].copy_from_slice(MAGIC); }
    ...
    fn native_dpapi_unavailable_or_invalid_never_falls_back() {
        ...
        assert!(matches!(transform(&[], false), Err(CryptoError::InputLimit)));
        assert!(matches!(transform(&vec![0; 4097], false), Err(CryptoError::InputLimit)));
    }
}
