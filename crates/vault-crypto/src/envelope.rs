...
use chacha20poly1305::{aead::{AeadInPlace, KeyInit}, Key, Tag, XChaCha20Poly1305, XNonce};
use std::fmt;
...
#[repr(u8)]
pub enum Purpose { Registry = 1, Identity = 2, Generation = 3, Journal = 4, CredentialResource = 5 }
...
impl EnvelopeContext {
    pub fn new(purpose: Purpose, schema: u32, profile: [u8; 16], generation: [u8; 16], resource: u16) -> Result<Self, CryptoError> {
        if schema == 0 || profile == [0; 16] || generation == [0; 16]
        ...
        Ok(Self { purpose, schema, profile, generation, resource })
    }
    ...
    pub(crate) fn check_length(&self, size: usize) -> Result<(), CryptoError> {
        let limit = if self.purpose == Purpose::CredentialResource { RESOURCE_LIMIT } else { MAX_PLAINTEXT_BYTES };
        if size > limit { Err(CryptoError::InputLimit) } else { Ok(()) }
    }
}
impl fmt::Debug for EnvelopeContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str("EnvelopeContext([REDACTED])") }
}
...
impl fmt::Debug for Envelope {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str("Envelope([REDACTED])") }
}
fn validate(bytes: &[u8]) -> Result<usize, CryptoError> {
    if bytes.len() > MAX_ENVELOPE_BYTES { return Err(CryptoError::InputLimit); }
    if bytes.len() < HEADER_BYTES + TAG_BYTES || &bytes[..4] != MAGIC { return Err(CryptoError::InvalidEnvelope); }
    if bytes[4] != 1 { return Err(CryptoError::UnsupportedVersion); }
    if bytes[5] != 1 { return Err(CryptoError::UnsupportedAlgorithm); }
    if bytes[6..22] == [0; 16] { return Err(CryptoError::InvalidEnvelope); }
    let len = u32::from_le_bytes(bytes[46..50].try_into().map_err(|_| CryptoError::InvalidEnvelope)?) as usize;
    if len != bytes.len() - HEADER_BYTES - TAG_BYTES { return Err(CryptoError::InvalidEnvelope); }
    Ok(len)
}
...
impl RootKey {
    ...
    pub fn seal(&self, context: &EnvelopeContext, plaintext: &[u8]) -> Result<Envelope, CryptoError> {
        self.seal_using(context, plaintext, &mut SystemEntropy)
    }
    fn seal_using(&self, context: &EnvelopeContext, plaintext: &[u8], rng: &mut impl Entropy) -> Result<Envelope, CryptoError> {
        ...
        let tag = cipher.encrypt_in_place_detached(XNonce::from_slice(&nonce), &aad(&header, context), &mut data)
    ...
    pub fn open(&self, context: &EnvelopeContext, envelope: &Envelope) -> Result<SecretBytes, CryptoError> {
        let bytes = envelope.as_bytes();
        let len = validate(bytes)?;
        context.check_length(len)?;
        if bytes[6..22] != self.id { return Err(CryptoError::AuthenticationFailed); }
        let key = self.derive(ENCRYPTION_INFO)?;
        let cipher = XChaCha20Poly1305::new(Key::from_slice(key.as_ref()));
        let mut data = Zeroizing::new(bytes[HEADER_BYTES..HEADER_BYTES + len].to_vec());
        cipher.decrypt_in_place_detached(XNonce::from_slice(&bytes[22..46]), &aad(&bytes[..HEADER_BYTES], context), &mut data,
...
mod tests {
    ...
    fn nonce_failure_never_returns_an_envelope() {
        ...
        let context = EnvelopeContext::new(Purpose::Generation, 1, [0x41;16], [0x42;16], 0).unwrap();
        assert!(matches!(key.seal_using(&context, b"SYNTHETIC", &mut FailingEntropy), Err(CryptoError::RandomnessUnavailable)));
    }
    #[test]
    ...
    fn independent_libsodium_envelope_vector_and_same_id_wrong_key() {
        ...
        let root = RootKey { secret: Zeroizing::new([0x53; 32]), id: [0x41; 16] };
        let ctx = EnvelopeContext::new(Purpose::Generation, 1, [0x41; 16], [0x42; 16], 0).unwrap();
        let sealed = root.seal_using(&ctx, b"CA03B_SYNTHETIC_NO_CREDENTIALS", &mut Fixed).unwrap();
        let expected: &[u8] = &[67,65,69,78,1,1,65,65,65,65,65,65,65,65,65,65,65,65,65,65,65,65,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,67,30,0,0,0,39,156,132,184,66,56,10,93,167,138,175,61,249,201,144,47,153,153,212,212,93,226,248,23,198,216,67,126,119,191,0,228,141,18,53,245,248,226,174,148,163,120,106,154,79,199];
        assert!(sealed.as_bytes() == expected);
        let wrong = RootKey { secret: Zeroizing::new([0x54; 32]), id: root.id };
        assert!(matches!(wrong.open(&ctx, &sealed), Err(CryptoError::AuthenticationFailed)));
    }
