use codex_accounts_vault_crypto::{CryptoError, Envelope, EnvelopeContext, ProtectedRootKey, Purpose, RootKey, MAX_ENVELOPE_BYTES, MAX_PLAINTEXT_BYTES};
...
fn opaque_bytes_roundtrip_for_every_purpose() {
    ...
    for purpose in [Purpose::Registry, Purpose::Identity, Purpose::Generation, Purpose::Journal, Purpose::CredentialResource] {
        ...
        assert!(!sealed.as_bytes().windows(plaintext.len()).any(|w| w == plaintext));
    }
}
...
fn every_context_field_and_purpose_is_authenticated() {
    ...
    for other in wrong {
        assert!(matches!(root.open(&other, &sealed), Err(CryptoError::AuthenticationFailed)));
    }
}
...
fn truncated_extended_overflowing_or_unknown_format_is_refused() {
    ...
    for (offset, expected) in [(4, CryptoError::UnsupportedVersion), (5, CryptoError::UnsupportedAlgorithm)] {
        let mut bytes = sealed.as_bytes().to_vec();
        bytes[offset] = 255;
    ...
    assert!(Envelope::from_bytes(&bytes).is_err());
    assert!(matches!(Envelope::from_bytes(&vec![0; MAX_ENVELOPE_BYTES + 1]), Err(CryptoError::InputLimit)));
}
...
fn wrong_root_key_and_stale_context_never_decrypt() {
    ...
    assert!(matches!(second.open(&ctx, &sealed), Err(CryptoError::AuthenticationFailed)));
    let later = EnvelopeContext::new(Purpose::Generation, 1, [0x41; 16], [0x44; 16], 0).unwrap();
    assert!(matches!(first.open(&later, &sealed), Err(CryptoError::AuthenticationFailed)));
...
fn exact_limits_succeed_and_one_extra_fails_before_encryption() {
    let root = RootKey::generate().unwrap();
    for (purpose, limit) in [(Purpose::CredentialResource, 1024 * 1024), (Purpose::Generation, MAX_PLAINTEXT_BYTES)] {
        let ctx = context(purpose);
        let bytes = vec![0x53; limit];
        let sealed = root.seal(&ctx, &bytes).unwrap();
        assert!(root.open(&ctx, &sealed).unwrap().expose() == bytes);
        assert!(matches!(root.seal(&ctx, &vec![0x53; limit + 1]), Err(CryptoError::InputLimit)));
    }
    let oversized_resource = root.seal(&context(Purpose::Generation), &vec![0x53; 1024 * 1024 + 1]).unwrap();
    assert!(matches!(root.open(&context(Purpose::CredentialResource), &oversized_resource), Err(CryptoError::InputLimit)));
}
...
fn fingerprints_bind_presence_bytes_context_and_key() {
    ...
    assert!(root.verify_fingerprint(&present, &ctx, Some(b"SYNTHETIC_ONLY")).is_ok());
    assert!(root.verify_fingerprint(&present, &ctx, Some(b"SYNTHETIC_OTHER")).is_err());
    assert!(root.verify_fingerprint(&present, &context(Purpose::Identity), Some(b"SYNTHETIC_ONLY")).is_err());
    assert!(other.verify_fingerprint(&present, &ctx, Some(b"SYNTHETIC_ONLY")).is_err());
    assert!(matches!(root.fingerprint(&ctx, Some(&vec![0; 1024 * 1024 + 1])), Err(CryptoError::InputLimit)));
}

#[test]
fn ambiguous_context_and_unbounded_protected_key_inputs_fail() {
    for (schema, profile, generation, slot) in [(0, [1; 16], [2; 16], 0), (1, [0; 16], [2; 16], 0), (1, [1; 16], [0; 16], 0), (1, [1; 16], [2; 16], 16)] {
        assert!(matches!(EnvelopeContext::new(Purpose::CredentialResource, schema, profile, generation, slot), Err(CryptoError::InvalidContext)));
    }
    assert!(EnvelopeContext::new(Purpose::Registry, 1, [1; 16], [2; 16], 1).is_err());
    for size in 0..11 { assert!(ProtectedRootKey::from_bytes(&vec![0; size]).is_err()); }
    assert!(matches!(ProtectedRootKey::from_bytes(&vec![0; 4097]), Err(CryptoError::InputLimit)));
...
fn other_hosts_do_not_get_a_plaintext_or_native_store_fallback() {
    let root = RootKey::generate().unwrap();
    assert!(matches!(root.protect(), Err(CryptoError::UnsupportedPlatform)));
    let wrapped = ProtectedRootKey::from_bytes(b"CAKP\x01\x01\x01\x00\x00\x00S").unwrap();
    assert!(matches!(RootKey::unprotect(&wrapped), Err(CryptoError::UnsupportedPlatform)));
}
