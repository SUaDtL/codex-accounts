...
use codex_accounts_vault_crypto::{
    CryptoError, Envelope, EnvelopeContext, ProtectedRootKey, Purpose, RootKey,
};
...
fn read_fixture() -> (ProtectedRootKey, Envelope) {
    let metadata = std::fs::symlink_metadata(FILE)
        .unwrap_or_else(|_| panic!("synthetic fixture metadata unavailable"));
    assert!(
        metadata.file_type().is_file() && metadata.len() <= 8192,
        "invalid synthetic fixture file"
    );
    let mut bytes = Vec::new();
    std::fs::File::open(FILE)
        .unwrap_or_else(|_| panic!("synthetic fixture unavailable"))
        .take(8193)
        .read_to_end(&mut bytes)
        .unwrap_or_else(|_| panic!("synthetic fixture read failed"));
    assert!(
        bytes.len() <= 8192 && bytes.len() >= MARKER.len() + 4,
        "invalid synthetic fixture size"
    );
    ...
    let start = offset + 4;
    assert!(
        size > 0 && size <= 4096 && start + size < bytes.len(),
        "invalid synthetic fixture structure"
    );
    let key = ProtectedRootKey::from_bytes(&bytes[start..start + size])
        .unwrap_or_else(|_| panic!("invalid protected synthetic key"));
    let envelope = Envelope::from_bytes(&bytes[start + size..])
        .unwrap_or_else(|_| panic!("invalid synthetic envelope"));
    (key, envelope)
}
...
fn native_two_user_1_create_fixture() {
    ...
    let mut file = std::fs::OpenOptions::new()
        ...
        .write(true)
        .open(FILE)
        .unwrap_or_else(|_| panic!("create a new synthetic fixture only"));
    file.write_all(MARKER)
        .unwrap_or_else(|_| panic!("synthetic fixture write failed"));
    file.write_all(&(protected.as_bytes().len() as u32).to_le_bytes())
        .unwrap_or_else(|_| panic!("synthetic fixture write failed"));
    ...
    file.write_all(sealed.as_bytes())
        .unwrap_or_else(|_| panic!("synthetic fixture write failed"));
    file.sync_all()
        .unwrap_or_else(|_| panic!("synthetic fixture flush failed"));
}
...
fn native_two_user_2_creator_control() {
    ...
    let root = RootKey::unprotect(&protected)
        .unwrap_or_else(|_| panic!("creator must unlock the same synthetic fixture"));
...
fn native_two_user_3_other_user_refuses() {
    ...
    assert!(
        matches!(
            RootKey::unprotect(&protected),
            Err(CryptoError::KeyProtectionUnavailable)
        ),
