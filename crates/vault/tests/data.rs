...
fn full_set_limit_is_aggregate_not_just_per_resource() {
    let rules: Vec<_> = (0..4)
        .map(|n| (slot(n), ResourceShape::Opaque, true))
        .collect();
    let resources = (0..4)
    ...
    let resources = (0..5)
        ...
        .collect();
    assert_eq!(
        CredentialSet::new(&rules, resources).unwrap_err(),
        DataError::InputLimit
    );
}
...
fn absence_is_not_empty_and_optional_absence_is_preserved() {
    let set = CredentialSet::new(
        &[
            (slot(0), ResourceShape::Opaque, false),
            (slot(1), ResourceShape::Opaque, true),
        ],
        vec![
            Resource::absent(slot(0)),
            Resource::present(slot(1), Vec::new()).unwrap(),
        ],
    )
    .unwrap();
...
fn omitted_extra_duplicate_and_required_absent_resources_fail() {
    ...
    assert_eq!(
        CredentialSet::new(&rule, vec![]).unwrap_err(),
        DataError::ResourceSetMismatch
    );
    assert_eq!(
        CredentialSet::new(&rule, vec![Resource::absent(slot(1))]).unwrap_err(),
        DataError::ResourceSetMismatch
    );
    assert_eq!(
        CredentialSet::new(
            &rule,
            vec![Resource::absent(slot(0)), Resource::absent(slot(0))]
        )
        .unwrap_err(),
        DataError::DuplicateResource
    );
    assert_eq!(
        CredentialSet::new(&rule, vec![Resource::absent(slot(0))]).unwrap_err(),
        DataError::RequiredResourceAbsent
    );
    assert_eq!(
        CredentialSet::new(&[rule[0], rule[0]], vec![]).unwrap_err(),
        DataError::DuplicateResource
    );
...
fn raw_resource_cannot_bypass_set_json_validation() {
    ...
    assert_eq!(
        CredentialSet::new(&[(slot(0), ResourceShape::JsonObject, true)], vec![bad]).unwrap_err(),
        DataError::InvalidJson
    );
}
...
fn empty_ambiguous_and_unbounded_identity_is_rejected() {
    for value in [
        ...
        assert_eq!(
            Identity::new("SYNTHETIC".into(), value, "SYNTHETIC".into()).unwrap_err(),
...
fn newest_generation_is_not_the_original_snapshot() {
    ...
    let mut index = GenerationIndex::new(
        profile(1),
        identity("SYNTHETIC_A", "SYNTHETIC_SPACE"),
        generation(1),
    );
    index
        .append(profile(1), &who, generation(1), generation(2))
        .unwrap();
    assert_eq!(index.latest(), generation(2));
    assert_eq!(
        index.append(profile(1), &who, generation(1), generation(3)),
...
fn mismatched_profile_or_identity_cannot_replace_a_generation() {
    ...
    assert_eq!(
        index.append(
            profile(1),
            &identity("SYNTHETIC_A", "OTHER_SPACE"),
            generation(1),
            generation(2)
        ),
        Err(DataError::IdentityMismatch)
...
fn generation_identifiers_cannot_be_reused() {
    ...
    assert_eq!(
        index.append(profile(1), &who, generation(1), generation(1)),
        Err(DataError::DuplicateGeneration)
    );
    assert_eq!(index.latest(), generation(1));
}
...
fn retention_decision_keeps_latest_and_all_supplied_unresolved_references() {
    ...
    index
        ...
        .unwrap();
    index
        .append(profile(1), &who, generation(2), generation(3))
        .unwrap();
    assert_eq!(
        index
            .unreferenced_candidates(&[generation(1), generation(1)])
            .unwrap(),
        vec![generation(2)]
    );
    assert!(index
        .unreferenced_candidates(&[generation(1), generation(2)])
        .unwrap()
        .is_empty());
    assert_eq!(
        index.unreferenced_candidates(&[generation(9)]),
        Err(DataError::UnknownGeneration)
    );
...
fn history_bound_refuses_growth_without_evicting_recovery_state() {
    ...
    for n in 2..=MAX_GENERATIONS as u8 {
        index
            .append(profile(1), &who, generation(n - 1), generation(n))
            .unwrap();
    }
    assert_eq!(
        index.append(profile(1), &who, generation(128), generation(129)),
        Err(DataError::InputLimit)
    );
    assert_eq!(index.latest(), generation(128));
    assert_eq!(
        index.unreferenced_candidates(&vec![generation(1); MAX_GENERATIONS + 1]),
        Err(DataError::InputLimit)
    );
}
