...
pub const ENV_FLAGS: &[&str] = &[
    "CODEX_HOME", "CODEX_CLI_PATH", "OPENAI_API_KEY", "OPENAI_BASE_URL", "CODEX_API_KEY",
...
impl fmt::Debug for Options {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str("Options([REDACTED])") }
}
...
pub fn options(args: impl IntoIterator<Item = String>) -> Result<Options, ReadError> {
    ...
    while let Some(arg) = args.next() {
        if !seen.insert(arg.clone()) { return Err(ReadError::Usage); }
        match arg.as_str() {
            "--candidate" => {
            ...
            "--candidate-home" => {
                let value = args.next().ok_or(ReadError::Usage)?;
                if value.is_empty() || value.len() > MAX_PATH_UNITS || value.contains('\0') { return Err(ReadError::Usage); }
...
pub fn validate_relative_executable(value: &str) -> Result<(), ReadError> {
    if value.len() > MAX_PATH_UNITS || value.is_empty() || value.starts_with(['/', '\\'])
        || value.contains([':', '\0', '%']) || !value.to_ascii_lowercase().ends_with(".exe") {
...
impl DeclaredConfig {
    pub fn unknown(state: &'static str) -> Self {
        Self { state, backend: Observation::Unknown, restriction_present: false,
            unsupported_source_present: false, profile_layers_present: false }
    }
...
pub fn parse_declared_config(bytes: &[u8]) -> DeclaredConfig {
    if bytes.len() > MAX_CONFIG_BYTES { return DeclaredConfig::unknown("input_limit"); }
    let Ok(text) = std::str::from_utf8(bytes) else { return DeclaredConfig::unknown("invalid_utf8"); };
    let Ok(table) = text.parse::<toml::Table>() else { return DeclaredConfig::unknown("invalid_toml"); };
    ...
    DeclaredConfig {
        state: "observed",
        backend,
        restriction_present: ["forced_login_method", "forced_chatgpt_workspace_id"].iter().any(|k| table.contains_key(*k)),
        unsupported_source_present: table.contains_key("model_providers")
            || table.get("model_provider").is_some_and(|v| v.as_str() != Some("openai"))
            || ["api_key", "experimental_bearer_token", "env_key"].iter().any(|k| table.contains_key(*k)),
...
pub fn context_report(config: &DeclaredConfig, flags: &[&str]) -> Value {
    let c = Context { declared_backend: config.backend, override_present: !flags.is_empty(), ..Context::default() };
...
pub fn encode(report: &Value) -> Result<String, ReadError> {
    ...
    if text.len() > MAX_REPORT_BYTES { return Err(ReadError::InputLimit); }
    Ok(text)
}
...
pub fn inspect(opts: &Options) -> Result<Value, ReadError> {
    ...
    #[cfg(not(windows))]
    { let _ = opts; Err(ReadError::UnsupportedPlatform) }
}
...
#[cfg(windows)]
pub(crate) fn path_text(path: &Path) -> String { path.to_string_lossy().into_owned() }
...
mod tests {
    ...
    fn declaration_is_not_precedence() {
        for mode in ["file", "auto", "keyring", "secrets", "ephemeral", "custom"] {
            let config = parse_declared_config(format!("cli_auth_credentials_store = '{mode}'").as_bytes());
    ...
    fn missing_invalid_denied_and_conflict_are_not_default_proof() {
        ...
        assert_eq!(parse_declared_config(b"cli_auth_credentials_store='file'\ncli_auth_credentials_store='auto'").state, "invalid_toml");
        assert_eq!(parse_declared_config(&[255]).state, "invalid_utf8");
        assert_eq!(parse_declared_config(&vec![b' '; MAX_CONFIG_BYTES + 1]).state, "input_limit");
        let cfg = DeclaredConfig { state: "inaccessible", backend: Observation::Inaccessible,
            ..DeclaredConfig::unknown("unknown") };
        assert_eq!(cfg.report()["backend_state"], "inaccessible");
        let cfg = DeclaredConfig { backend: Observation::Conflicting, ..cfg };
    ...
    fn reports_and_errors_never_include_config_or_argument_canaries() {
        ...
        assert!(cfg.restriction_present && cfg.unsupported_source_present && cfg.profile_layers_present);
        assert!(!format!("{cfg:?}").contains(canary));
        assert!(!encode(&context_report(&cfg, &[])).unwrap().contains(canary));
        let options = options(vec!["--candidate".into(), "a".repeat(64), "--candidate-home".into(), canary.into()]).unwrap();
        assert!(!format!("{options:?}").contains(canary));
        assert!(!encode(&error_report(ReadError::Usage)).unwrap().contains(canary));
    }

    #[test]
    fn relative_paths_cannot_escape_package_or_name_credentials() {
        for bad in ["../x.exe", "C:\\x.exe", "\\\\server\\x.exe", "a//b.exe", "a/./b.exe", "a/../b.exe", "auth.json", "cap_sid", "a.exe:auth.json", "a. /b.exe", "CON.exe", "a/%x%.exe", "a/../auth.exe", "a\0.exe"] {
            assert_eq!(validate_relative_executable(bad), Err(ReadError::UnsafePath), "synthetic invalid path accepted");
        }
    ...
    fn options_do_not_offer_authority_or_arbitrary_invocation() {
        for bad in ["--force", "--qualified", "--rpc", "--execute", "--auth-file", "--publisher"] {
            assert!(options(vec![bad.into(), "SYNTHETIC_CANARY".into()]).is_err());
        }
        assert!(options(vec!["--candidate".into(), "a".repeat(64), "--candidate".into(), "a".repeat(64)]).is_err());
    }
    ...
    fn output_is_bounded_and_json_escaped() {
        ...
        assert_eq!(serde_json::from_str::<Value>(&text).unwrap()["value"], "<script>\"\nSYNTHETIC");
    }
}
