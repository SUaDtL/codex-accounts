...
impl Apartment {
    pub fn enter() -> Result<Self, ReadError> {
        ...
        if result.is_err() {
            return Err(ReadError::EnumerationUnavailable);
        }
...
#[cfg(test)]
thread_local! {
    static OPENED_PATHS: std::cell::RefCell<Vec<PathBuf>> = const { std::cell::RefCell::new(Vec::new()) };
}
#[cfg(test)]
pub(crate) fn take_opened_paths() -> Vec<PathBuf> {
    OPENED_PATHS.with(|paths| std::mem::take(&mut *paths.borrow_mut()))
}

fn open_one(path: &Path, directory: bool) -> Result<File, ReadError> {
    #[cfg(test)]
    OPENED_PATHS.with(|paths| paths.borrow_mut().push(path.to_owned()));
    let name = wide(path)?;
...
impl std::fmt::Debug for ReadHandle {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("ReadHandle([REDACTED])")
    }
}
impl ReadHandle {
    pub fn open(path: &Path, directory: bool) -> Result<Self, ReadError> {
        ...
        chain.reverse();
        for parent in chain {
            ancestors.push(open_one(parent, true)?);
        }
        let file = open_one(path, directory)?;
        let initial = stamp(&file)?;
        if !directory && initial.size > MAX_EXECUTABLE_BYTES {
    ...
    pub fn bytes(&mut self, limit: usize) -> Result<Vec<u8>, ReadError> {
        ...
        (&mut self.file)
            ...
            .map_err(|_| ReadError::Io)?;
        if bytes.len() > limit || bytes.len() as u64 != self.initial.size {
            return Err(ReadError::Changed);
        }
        self.revalidate()?;
        Ok(bytes)
    ...
    pub fn current_user_can_replace_or_modify(&self) -> Result<bool, ReadError> {
        if writable(&self.file, self.directory)? {
            return Ok(true);
        }
        for ancestor in &self.ancestors {
            if writable(ancestor, true)? {
                return Ok(true);
            }
        }
        Ok(false)
    ...
    pub fn cached_signature(&self) -> bool {
        let Ok(name) = wide(&self.path) else {
            return false;
        };
...
fn writable(file: &File, directory: bool) -> Result<bool, ReadError> {
    ...
    if unsafe { DuplicateToken(process_token.0, SecurityImpersonation, &mut token) } == 0 {
        return Err(last_error());
    }
    ...
    if access == 0 {
        return Err(ReadError::AccessDenied);
    }
    let unsafe_rights = if directory {
...
mod tests {
    ...
    fn native_hardlinks_and_sharing_writers_are_refused() {
        ...
        assert!(matches!(
            ReadHandle::open(&path, false),
            Err(ReadError::SharingViolation)
        ));
        drop(writer);
    ...
    fn native_directories_and_data_bounds_are_checked() {
        let fixture = Fixture::new();
        assert!(matches!(
            ReadHandle::open(&fixture.0, false),
            Err(ReadError::UnsafePath)
        ));
        let path = fixture.0.join("config.toml");
        std::fs::write(&path, b"SYNTHETIC_DATA").unwrap();
        assert_eq!(
            ReadHandle::open(&path, false).unwrap().bytes(2),
            Err(ReadError::InputLimit)
        );
        for path in [
            "relative.exe",
            "\\\\server\\share\\x.exe",
    ...
    #[test]
    fn native_denied_read_is_not_absence() {
        use std::os::windows::fs::OpenOptionsExt;
        let fixture = Fixture::new();
        let path = fixture.0.join("denied.exe");
        std::fs::write(&path, b"SYNTHETIC_DATA").unwrap();
        ...
        // SAFETY: Test owns this file and keeps a control handle across the
        // temporary denial. Restore the original ACL before checking assertions.
        let got = unsafe {
            GetSecurityInfo(
        ...
        assert_eq!(applied, ERROR_SUCCESS);
        let result = ReadHandle::open(&path, false);
        let restored = unsafe {
            SetSecurityInfo(
                control.as_raw_handle(),
                SE_FILE_OBJECT,
        ...
        assert!(matches!(result, Err(ReadError::AccessDenied)));
        assert_eq!(std::fs::read(&path).unwrap(), b"SYNTHETIC_DATA");
    }

    #[test]
    fn native_junction_leaf_and_ancestor_are_rejected() {
        ...
        std::fs::write(target.join("synthetic.exe"), b"SYNTHETIC_DATA").unwrap();
        let name = wide(&junction).unwrap();
        // SAFETY: This write is restricted to an empty directory created by the
        // test. The production reader never creates a reparse point or writes.
        let raw = unsafe {
            CreateFileW(
                name.as_ptr(),
                GENERIC_WRITE,
        ...
        let substitute_len = (substitute.len() * 2) as u16;
        ...
        for value in [0, substitute_len, substitute_len + 2, print_len] {
        ...
        let mut returned = 0;
        // SAFETY: Synchronous ioctl with bounded byte buffer using the documented
        // mount-point REPARSE_DATA_BUFFER layout. No output buffer is requested.
        ...
        let leaf = ReadHandle::open(&junction, true);
        let child = ReadHandle::open(&junction.join("synthetic.exe"), false);
        std::fs::remove_dir(&junction).unwrap();
        assert!(matches!(leaf, Err(ReadError::UnsafePath)));
        assert!(matches!(child, Err(ReadError::UnsafePath)));
        assert_eq!(
            std::fs::read(target.join("synthetic.exe")).unwrap(),
            b"SYNTHETIC_DATA"
        );
    }
}
