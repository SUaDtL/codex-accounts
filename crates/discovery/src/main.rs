...
fn main() {
    let args: Result<Vec<String>, ReadError> = std::env::args_os().skip(1)
        .map(|arg| arg.into_string().map_err(|_| ReadError::Usage)).collect();
    if args.as_ref().is_ok_and(|args| args == &["--help"]) {
        println!("Read-only Windows registered-package discovery. Never account authority.\n\
    ...
    let (text, failed) = match encode(&report) {
        Ok(text) => (text, failed),
        Err(error) => (encode(&error_report(error)).expect("fixed error JSON"), true),
    };
    if writeln!(io::stdout().lock(), "{text}").is_err() || failed { std::process::exit(2); }
}
