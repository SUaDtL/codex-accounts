...
class DependencyReviewTests(unittest.TestCase):
    def setUp(self):
        ...
        manifest = json.loads((ROOT / 'docs/ca-04b-dependencies.json').read_text())
        paths = list(manifest['manifest_sha256']) + list(manifest['predecessor_sha256']) + ['Cargo.lock', 'docs/ca-04b-dependencies.json']
    ...
    def fail_with(self, review):
        (self.root / 'docs/ca-04b-dependencies.json').write_text(json.dumps(review))
